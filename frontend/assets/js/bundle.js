...
function hideErrorMessages(formulary) {
...
function checkForEmptyFields(...inputs) {
    inputs.forEach((input) => {
        if (!input.value)
            showErrorMessage(input, 'Campo não pode ficar vázio');
    });
}
function checkEmail(input) {
    if (!(0, isEmail_1.default)(input.value))
        showErrorMessage(input, 'Insira um e-mail válido !');
}
function checkTwoPasswords(...inputs) {
    if (inputs[0].value !== inputs[1].value)
        showErrorMessage(inputs[1], 'Confirmação de senha divergente');
}
function shouldSendForm() {
    if (!document.querySelector(`.${SHOW_ERROR_MESSAGES}`))
        return true;
    return false;
}
